...
class EQAPBox:
    ...
    def __init__(self):
        ...
        if self.PublicMethod.GetInitWithError() == True:
        ...
        print(self.Device.ServerIP,self.Device.ServerPort)
            ...
            try:
                ...
                while nQueueSize > 0:
                    ...
                    for sData in lstData:
                        if self.ProtocolMgmt.CheckMsgIsDefined(sData, nProtocolID) == False:
                            continue
                        timeStr = time.strftime("%Y/%m/%d %H:%M:%S", time.localtime())
                        bSaveData = self.Sqlite.SaveCommDataToDB((sData, 0, nProtocolID, strInstrumentName, 0,timeStr))
            except Exception:
                self.PublicMethod.Log('Process instrument communiction data failed')
    ...
    def ProcessDBData(self):
        while True:
            ...
            if self.SysConfig.LastDataCleanDate < dtCurDate:
                ...
                if bCleanStatus == True:
                    self.SysConfig.LastDataCleanDate = dtCurDate
                    self.Sqlite.UpdateSysConfig(int(self.SysConfig.SaveLog),self.SysConfig.TerminalID,self.SysConfig.TimeSequence,self.SysConfig.RetryTimes,self.SysConfig.DataKeptDays,self.SysConfig.LastDataCleanDate)
            ...
            for data in dataList:
                ...
                if len(dataSendToServer) == 0:
                    self.Sqlite.RevmoeCommDataFromDB(data.SerialNo)
                    sleep(0.1)
                    continue
    ...
    def UnFormatData(self,dataFromServer):
        ...
        byteError = dataDecrypt[2:nIndex]
        errorInfo = str(byteError,encoding='utf-8').strip()

        if errorInfo == "ErrorID\t0":
            return 0
        else:
            print("ErrorInfo:",dataToDeCrypt[0:30])
            return 1
        # return dataDecrypt
...
if __name__ == "__main__":
    ...
    # for instr in instrs:
    #     print(instr.InstrumentName,instr.InstrumentSerialNo,instr.InstrumentDesc,instr.InstrumentLocation,instr.HighConcentrationID,instr.MidConcentrationID,instr.LowConcentrationID,instr.ProtocolID,instr.BaudRate,instr.PortName,instr.IsActive)
    ...
    # #/dev/ttyXRUSB0
    # #/dev/ttyXRUSB0
    # sqlite.ExecuteSqlCmd('''UPDATE  BSC_InstrumentInfo SET PortName = 'com1',IsActive = True WHERE InstrumentSerialNo = 'AU5800-1_SerialNo' ''')
