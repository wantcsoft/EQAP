...
class SqliteHelper:
    ...
    def SaveInstrumentInfoToDB(self,model):
        sqlCmdInsert = '''INSERT INTO BSC_InstrumentInfo(InstrumentName,InstrumentSerialNo,InstrumentDesc,InstrumentLocation,HighConcentrationID,MidConcentrationID,LowConcentrationID,ProtocolID,BaudRate,PortName,IsActive
        )VALUES(?,?,?,?,?,?,?,?,?,?,?);'''
    ...
    '''
    def UpdateInstrumentInfoToDB(self,nInstrNo,strInstrName,strInstrSerialNo,strInstrDesc,strInstrLocation,strHighConcentID,strMidConcentID,strLowConcentID,nProtocolID,nBaudRate,strPortName,nIsActive):
        bRetValue = False
        sqlCmdUpdate = '''UPDATE BSC_InstrumentInfo SET InstrumentName = '{}', InstrumentSerialNo = '{}', InstrumentDesc = '{}',InstrumentLocation = '{}',HighConcentrationID = '{}',MidConcentrationID = '{}',LowConcentrationID = '{}',ProtocolID = {}, BaudRate = {}, PortName ='{}',IsActive = {} WHERE InstrumentNo = {} '''.format(strInstrName,strInstrSerialNo,strInstrDesc,strInstrLocation,strHighConcentID,strMidConcentID,strLowConcentID,nProtocolID,nBaudRate,strPortName,nIsActive,nInstrNo)
        try:
            conn = sqlite3.connect(self.DbFileName)
            cursor = conn.cursor()
    ...
    def GetInstrumentInfoFromDB(self):
        sqlCmdQuery = '''SELECT InstrumentNo,InstrumentName,InstrumentSerialNo,InstrumentDesc,InstrumentLocation,HighConcentrationID,MidConcentrationID,LowConcentrationID,ProtocolID,BaudRate,PortName,IsActive FROM BSC_InstrumentInfo'''
        ...
        try:
            ...
            for instr in instrs:
                ...
                model.InitInstrumentInfo(instr[0],instr[1],instr[2],instr[3],instr[4],instr[5],instr[6],instr[7],instr[8],instr[9],instr[10],instr[11])
    ...
    def GetChemCode(self):
        sqlCmdTxt = '''SELECT ChemCode,TestItemCode FROM BSC_ChemCode'''
        lstChemCode = []
    ...
    def UpdateChemCode(self,model):
        sqlCmdTxt = '''UPDATE BSC_ChemCode SET TestItemCode = '{}' WHERE ChemCode = '{}' '''.format(model.TestItemCode,model.ChemCode)
        bRetValue = False
        try:
