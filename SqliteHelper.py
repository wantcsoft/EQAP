...
class SqliteHelper:
    ...
    def SaveSysConfig(self, nSaveLog, nTerminalID, nTimeSequence, nRetryTims, nDataKeptDays, dtLastCleanDate):
        sqlCmdTxt = "INSERT INTO BSC_SysConfig(SaveLog,TerminalID,TimeSequence,RetryTims,DataKeptDays)VALUES({},{},{},{})".format(
            nSaveLog, nTerminalID, nTimeSequence, nDataKeptDays)
        conn = sqlite3.connect(self.DbFileName)
    ...
    def UpdateSysConfig(self, nSaveLog, nTerminalID, nTimeSequence, nRetryTims, nDataKeptDays, dtLastCleanDate):
        sqlCmdTxt = "UPDATE BSC_SysConfig SET SaveLog = {}, TerminalID = {}, TimeSequence = {}, RetryTims = {},DataKeptDays={},LastDataCleanDate='{}'".format(
            nSaveLog, nTerminalID, nTimeSequence, nRetryTims, nDataKeptDays, dtLastCleanDate)
        conn = sqlite3.connect(self.DbFileName)
        conn.execute(sqlCmdTxt)
        conn.commit()
        conn.close()
    ...
    def UpdateResendTimes(self, nSerialNo, nTimes, nIsUploaded):
        bRetValue = False
        sqlCmdTxt = '''UPDATE WOR_EqapCommInfo SET RetryTimes = {},IsUploaded = {} WHERE SerialNo={}'''.format(nTimes,
                                                                                                               nIsUploaded,
                                                                                                               nSerialNo)
        conn = sqlite3.connect(self.DbFileName)
        try:
    ...
    def GetCommDataFromDB(self, nMaxRowCount, nRetryTimes):
        ...
        if nMaxRowCount <= 0:
            sqlCmdQuery = '''SELECT SerialNo, DetailedInfo,IsUploaded, ProtocolID,InstrumentName, RetryTimes,CreateTime FROM WOR_EqapCommInfo WHERE IsUploaded = 0 and RetryTimes < {maxReTryTimes} ORDER BY SerialNo asc LIMIT 100'''.format(
                maxReTryTimes=nRetryTimes)
        else:
            sqlCmdQuery = '''SELECT SerialNo, DetailedInfo,IsUploaded, ProtocolID,InstrumentName, RetryTimes,CreateTime FROM WOR_EqapCommInfo WHERE IsUploaded = 0 and RetryTimes < {maxReTryTimes}  ORDER BY SerialNo asc LIMIT {rowCount}'''.format(
                rowCount=nMaxRowCount, maxReTryTimes=nRetryTimes)
    ...
    返回值：
        ...
        False：删除失败
    '''
    def RemoveCommDataFromDB(self, nSerialNo):
        bRetValue = False
        try:
    ...
    def ClearCommDataFromDB(self, nSerialNo):
        bRetValue = False
        conn = sqlite3.connect(self.DbFileName)
        try:
    ...
    '''
    def UpdateInstrumentInfoToDB(self, nInstrNo, strInstrID, strInstrName, strInstrSerialNo, strInstrDesc,
                                 strInstrLocation, strHighConcentID, strMidConcentID, strLowConcentID, nProtocolID,
                                 nBaudRate, strPortName, nIsActive):
        bRetValue = False
        sqlCmdUpdate = '''UPDATE BSC_InstrumentInfo SET InstrumentID = '{}' InstrumentName = '{}', InstrumentSerialNo = '{}', InstrumentDesc = '{}',InstrumentLocation = '{}',HighConcentrationID = '{}',MidConcentrationID = '{}',LowConcentrationID = '{}',ProtocolID = {}, BaudRate = {}, PortName ='{}',IsActive = {} WHERE InstrumentNo = {} '''.format(
            strInstrName, strInstrSerialNo, strInstrDesc, strInstrLocation, strHighConcentID, strMidConcentID,
            strLowConcentID, nProtocolID, nBaudRate, strPortName, nIsActive, nInstrNo)
        conn = sqlite3.connect(self.DbFileName)
        cursor = conn.cursor()
        try:
    ...
    def GetInstrumentInfoFromDB(self):
        ...
        try:
            ...
            for instr in instrs:
                ...
                model.InitInstrumentInfo(instr[0], instr[1], instr[2], instr[3], instr[4], instr[5], instr[6], instr[7],
                                         instr[8], instr[9], instr[10], instr[11], instr[12])
    ...
    def SaveProtocolInfo(self, model):
        sqlCmdTxt = '''INSERT INTO BSC_ProtocolInfo(ProtocolName,ProtocolType,IsEnabled)VALUES('{}','{}',{})'''.format(
            model.ProtocolName, model.ProtocolType, model.IsEnabled)
    ...
    def UpdateProtocolInfo(self, model):
        ...
        conn = sqlite3.connect(self.DbFileName)
        try:
            sqlCmdTxt = '''UPDATE BSC_ProtocolInfo SET ProtocolName = '{}',ProtocolType='{}',IsEnabled = {} WHERE ProtocolID = {}'''.format(
                model.ProtocolName, model.ProtocolType, model.IsEnabled, model.ProtocolID)
            conn.execute(sqlCmdTxt)
            conn.commit()
            bRetValue = True
        except BaseException as e:
    ...
    def SaveProtocolMsgInfo(self, model):
        ...
        conn = sqlite3.connect(self.DbFileName)
        try:
            sqlSaveCmdTxt = '''INSERT INTO BSC_ProtocolMsgInfo(ProtocolID,ProtocolMsgType,MsgHeader,MsgEndFlag,FieldCount,UploadFormat,ToBeUpload,SampleIDFrom)VALUES({},'{}','{}','{}',{},'{}',{},'{}')'''.format(
                model.ProtocolID, model.MsgTypeID, model.MsgHeader, model.MsgEnd, model.FieldCount, model.UploadFormat,
                model.ToBeUpload, model.SampleIDFrom)
    ...
    def UpdateProtocolMsgInfo(self, model):
        ...
        try:
            sqlCmdTxt = '''UPDATE BSC_ProtocolMsgInfo SET ProtocolMsgType={},MsgHeader='{}', MsgEndFlag = '{}', FieldCount = {}, UploadFormat = '{}', ToBeUpload = {},SampleIDFrom='{}' WHERE ProtocolID = {} AND MsgID = {} '''.format(
                model.MsgTypeID, model.MsgHeader, model.MsgEnd, model.FieldCount, model.UploadFormat, model.ToBeUpload,
                model.SampleIDFrom, model.ProtocolID, model.MsgID)
    ...
    def RemoveProtocolMsgInfo(self, nProtocolID, nMsgID):
        ...
        try:
            sqlCmdTxt = '''DELETE FROM BSC_ProtocolMsgInfo WHERE ProtocolID = {} AND MsgID = {} '''.format(nProtocolID,
                                                                                                           nMsgID)
            conn = sqlite3.connect(self.DbFileName)
            conn.execute(sqlCmdTxt)
            # conn.commit()

            sqlCmdTxt = '''DELETE FROM BSC_BasicProtocolFieldInfo WHERE ProtocolID = {} AND MsgID = {}'''.format(
                nProtocolID, nMsgID)
            conn.execute(sqlCmdTxt)
            conn.commit()
    ...
    def SaveProtocolMsgFieldInfo(self, model):
        ...
        conn = sqlite3.connect(self.DbFileName)
        try:
            sqlSaveCmdTxt = ''' INSERT INTO  BSC_BasicProtocolFieldInfo(ProtocolID,MsgID,FieldName,FieldType,FieldIndex,StartPos,FieldLen,ValueRange,ToBeCleared,IsFixed,IsRepeat,IsUpload)VALUES({},{},'{}','{}',{},{},{},'{}',{},{},{},{})'''.format(
                model.ProtocolID, model.MsgID, model.FieldName, model.FieldType, model.FieldIndex, model.StartPos,
                model.FieldLen, model.ValueRange, model.ToBeCleared, model.IsFixed, model.IsRepeat, model.IsUpload)
    ...
    def UpdateProtocolMsgFieldInfo(self, model):
        ...
        conn = sqlite3.connect(self.DbFileName)
        try:
            sqlCmdTxt = '''UPDATE BSC_BasicProtocolFieldInfo SET FieldName = '{}', FieldType = '{}',FieldIndex={}, StartPos = {}, FieldLen = {}, ValueRange = '{}', ToBeCleared={},IsFixed = {},IsRepeat = {},IsUpload = {} WHERE ProtocolID={} AND MsgID={} AND FieldID = {} '''.format(
                model.FieldName, model.FieldType, model.FieldIndex, model.StartPos, model.FieldLen, model.ValueRange,
                model.ToBeCleared, model.IsFixed, model.IsRepeat, model.IsUpload, model.ProtocolID, model.MsgID,
                model.FieldID)
    ...
    def RemoveProtocolMsgFieldInfo(self, nProtocolID, nMsgID, nFieldID):
        bRetValue = False
        try:
            sqlCmdTxt = "DELETE FROM BSC_BasicProtocolFieldInfo WHERE ProtocolID = {} AND MsgID = {} AND FieldID = {}".format(
                nProtocolID, nMsgID, nFieldID)
            conn = sqlite3.connect(self.DbFileName)
            conn.execute(sqlCmdTxt)
            conn.commit()
            bRetValue = True
            ...
            bRetValue = False
            # self.Logger.error(
            #     "Remove protocol:{} message:{} field:{} info with EXCEPTION:{}".format(nProtocolID, nMsgID, nFieldID,
            #                                                                            e))
            print(e)
        return bRetValue
    ...
    def GetProtocolMsgFieldInfo(self):
        ...
        try:
            ...
            for r in result:
                ...
                model.InitProtocolMsgFieldInfo(r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9], r[10], r[11],
                                               r[12])
    ...
    def UpdateChemCode(self, model):
        sqlCmdTxt = '''UPDATE BSC_ChemCode SET TestItemCode = '{}' WHERE ChemCode = '{}' AND InstrumentName = '{}' '''.format(
            model.InstrumentName, model.TestItemCode, model.ChemCode)
        bRetValue = False
        conn = sqlite3.connect(self.DbFileName)
        try:
            conn.execute(sqlCmdTxt)
    ...
    def SaveQCSampleInfo(self, model):
        ...
        conn = sqlite3.connect(self.DbFileName)
        try:
            sqlCmdTxt = '''INSERT INTo BSC_QCSampleInfo(QCSampleNo,QCSampleType,InstrumentNo)VALUES('{}','{}',{})'''.format(
                model.QCSampleNo, model.QCSampleType, model.InstrumentNo)
            print(sqlCmdTxt)
            conn.execute(sqlCmdTxt)
            conn.commit()
            bRetValue = True
    ...
    def DeleteQCSampleInfo(self, model):
        ...
        conn = sqlite3.connect(self.DbFileName)
        try:
            sqlCmdTxt = '''DELETE FROM BSC_QCSampleInfo WHERE QCSampleNo = {} and InstrumentNo = {}'''.format(
                model.QCSampleNo, model.InstrumentNo)
